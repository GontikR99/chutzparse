...
func handleHeal(mp *multipattern.Multipattern) *multipattern.Multipattern {
	return commonSubpatterns(mp).
		On("(.+) has been healed over time for (@num@) (?:\\((@num@)\\) )?hit points(?: by (.+))?[.](@hflag@)?",
		...
		On("(.*) healed (.*) for (@num@) (?:\\((@num@)\\) )?hit points(?: by (.*))?[.](@hflag@)?",
			func(parts []string, _ interface{}) interface{} {
		...
		On("The gods have healed (.*) for (@num@) points of damage[.]", func(parts []string, _ interface{}) interface{} {
			return &HealLog{
				Source:    UnspecifiedName,
				Target:    parts[1],
